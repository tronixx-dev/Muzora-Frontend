      ...
      {songs.length > 0 && (
        <section className="mb-8 md:mb-10">
          <div className="flex items-center justify-between mb-4 md:mb-5">
            ...
            <Link href="/trending" className="text-gray-400 text-sm hover:text-white transition-colors font-medium">
              Show all
            </Link>
          </div>
      ...
      {artists.length > 0 && (
        <section className="mb-8 md:mb-10">
          ...
          <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 md:gap-5">
            {artists.map((artist) => (
              <Link key={artist._id} href={`/artists/${artist._id}`} className="flex flex-col items-center gap-2 group cursor-pointer">
                <div className="w-full aspect-square rounded-full overflow-hidden bg-dark-100 ring-2 ring-transparent group-hover:ring-green-500 transition-all duration-300">
                  <img src={artist.imageUrl || '/placeholder.png'} alt={artist.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" />
                </div>
